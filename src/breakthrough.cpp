...
void Breakthrough::initialize()
{
  ...
  for(size_t k = 1; k < indexes.size(); ++k) {
    for(size_t i = indexes[k-1]; i < indexes[k]; ++i) {
      ...
      iastPerformance += mixture.predictMixture(LayerCounterSite, Yi, pt_init[i], Xi, Ni, &cachedP0[i * Ncomp * maxIsothermTerms],
  ...
  for (size_t step = 0; (step < Nsteps || autoSteps); ++step)
  {
    ...
    if (step % printEvery == 0)
    {
      ...
      std::cout << "    Average number of mixture-prediction steps: " +
                ...
                << std::endl;
      std::cout << "Current middle-point Temperature: " + std::to_string(Tgsnew[mid_index]) << " K" << std::endl;
    }
  }
...
void Breakthrough::computeStep(size_t step)
{
  ...
  if (autoSteps)
  {
    ...
    {
      if (IsothermalRegime) {
        tolerance = std::max(tolerance, std::abs((P[Ngrid * Ncomp + j] / ((p_total + dptdx * L) * components[j].Yi0)) - 1.0));
      }
      else {
        tolerance = std::max(tolerance, std::abs((P[Ngrid * Ncomp + j] / ((p_total + dptdx * L) * components[j].Yi0)) - 1));
      }
    }
