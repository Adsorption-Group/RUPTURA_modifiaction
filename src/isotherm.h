    ...
    switch (type)
    {
      ...
      case Isotherm::Type::Langmuir_Freundlich:
      {
        if (parameters.size() == 3) {
          ...
          return (parameters[0] + parameters[3] * T) / (parameters[2] + parameters[5] / T) * std::log(1.0 + parameters[1] * std::exp(parameters[4] / T) * std::pow(pressure, parameters[2] + parameters[5] / T));
        }
        return 0;
      }
      case Isotherm::Type::Langmuir_Freundlich_T_exp:
