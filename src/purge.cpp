...
Purge::Purge(const InputReader &inputReader)
    : displayName(inputReader.displayName),
      ...
      writeEvery(inputReader.writeEveryPurge),
      T(inputReader.temperature),
      Tamb(inputReader.Tamb),
      p_total(inputReader.Pmin),
...
{
  auto getPhysicalLayer = [&](size_t i) -> size_t {
      if (numberOfLayers <= 1) return 0;
      // Если это часть цикла, массивы перевернуты, ищем с конца. Иначе - с начала.
      size_t physical_i = cycle ? (Ngrid - i) : i;
      for (size_t k = 0; k < numberOfLayers; ++k) {
          if (physical_i < indexes[k + 1]) return k;
      }
  ...
  for (size_t j = 0; j < Ncomp; ++j) {
    ...
    if (components[j].isCarrierGas) {
        if (components[j].D_values.empty()) components[j].D_values.resize(numberOfLayers, 0.0);
        if (components[j].dH_values.empty()) components[j].dH_values.resize(numberOfLayers, 0.0);
    }
  }
  ...
  for (size_t i = 0; i < Ngrid + 1; ++i) {
    size_t layerCounter = getPhysicalLayer(i);
    for (size_t j = 0; j < Ncomp; ++j) {
  ...
  if (!cycle) {
    ...
    std::string fileName = "C:\\InstituteWork\\ruptura_modification_examples\\Cycles_tests\\BlowDown\\testPurgeNew.txt";
  ...
  for(size_t i = 0; i < Ngrid + 1; ++i) {
      ...
      if (sum > 1e-12) {
          for (size_t j = 0; j < Ncomp; ++j) Yi[j] /= sum;
      } else {
          for (size_t j = 0; j < Ncomp; ++j) Yi[j] = 0.0;
          
      }

      iastPerformance += mixture.predictMixture(layerCounter, Yi, Pt[i], Xi, Ni, &cachedP0[i * Ncomp * maxIsothermTerms],
  ...
  // for (size_t i = 0; i < Ngrid + 1; ++i) {
  //   for (size_t j = 0; j < Ncomp; ++j) {
  //     std::cout << Qeq[i * Ncomp + j] << " " << Q[i * Ncomp + j] << std::endl;
  //   }
  // }
...
void Purge::computeStep(size_t step)
{
  double t = static_cast<double>(step) * dt;
...
void Purge::computeEquilibriumLoadings()
{
  auto getPhysicalLayer = [&](size_t i) -> size_t {
      if (numberOfLayers <= 1) return 0;
      size_t physical_i = cycle ? (Ngrid - i) : i;
...
void Purge::computeFirstDerivatives2(
    std::vector<double> &dqdt, std::vector<double> &dpdt, std::vector<double> &dTdt, std::vector<double> &dTdtWall,
    const std::vector<double> &q_eq, const std::vector<double> &q, const std::vector<double> &v, const std::vector<double> &p, 
    const std::vector<double> &Tmp, std::vector<double> &TmpWall)
{
    ...
    for (size_t i = 1; i < Ngrid; i++) {
        ...
        double DenomTerm = (Pt[i] / (R * Tmp[i])) * eps[layerCounter] * Cpg_mix[i] 
                         + (1.0 - eps[layerCounter]) * rho_particle[layerCounter] * Cps_layer[layerCounter];
    ...
    {
        size_t layerCounter = getPhysicalLayer(Ngrid);
        double sumH = 0.0;
        ...
        for (size_t j = 0; j < Ncomp; ++j) {
             ...
             dpdt[Ngrid * Ncomp + j] = 
                ...
                + (p[Ngrid * Ncomp + j] / Tmp[Ngrid]) * dTdt[Ngrid]; 
        }
...
void Purge::computeCpgMix(std::vector<double> &Pi) {
  std::fill(Cpg_mix.begin(), Cpg_mix.end(), 0.0);
  for (size_t i = 0; i < Ngrid + 1; ++i) {
    double local_Pt = Pt[i]; 
    if (local_Pt < 1e-6) local_Pt = 1e-6; 

    for (size_t j = 0; j < Ncomp; ++j) { 
       Cpg_mix[i] += components[j].Cpg * Pi[i * Ncomp + j] / local_Pt; 
    }
  }  
}

void Purge::computeVelocityTemperatureLight()
{
    double idx2 = 1.0 / (dx * dx);
    Vnew[0] = v_in;
    double dPdz = 0;
    ...
    for (size_t i = 1; i < Ngrid; ++i) {
        ...
        for (size_t j = 0; j < Ncomp; ++j) {
            sum = sum - prefactors[i * Ncomp + j] * Tgsnew[i] * (Qeqnew[i * Ncomp + j] - Qnew[i * Ncomp + j]) +
        ...
        Vnew[i] = Vnew[i - 1] + dx * (sum - Vnew[i - 1] * dPdz) / Pt[i] + thermal_expansion + dx * (DTdtnew[i] / Tgsnew[i]);
      
        //std::cout << dx * (sum - Vnew[i - 1] * dPdz) / Pt[i] << " " << i << " " << Vnew[i] << " " << Vnew[i - 1] << std::endl;
    }

    size_t layerCounterN = getPhysicalLayer(Ngrid);
    double sum = 0.0;
    for (size_t j = 0; j < Ncomp; ++j) {
        sum = sum - prefactors[Ngrid * Ncomp + j] * Tgsnew[Ngrid] * (Qeqnew[Ngrid * Ncomp + j] - Qnew[Ngrid * Ncomp + j]) +
              components[j].D_values[layerCounterN] * (Pnew[(Ngrid - 1) * Ncomp + j] - Pnew[Ngrid * Ncomp + j]) * idx2;
    }
    Vnew[Ngrid] = Vnew[Ngrid - 1] + dx * (sum - Vnew[Ngrid - 1] * dPdz) / Pt[Ngrid];

}

void Purge::Kl_update(std::vector<double> &Tmpgs)
{
  auto getPhysicalLayer = [&](size_t i) -> size_t {
      if (numberOfLayers <= 1) return 0;
      size_t physical_i = cycle ? (Ngrid - i) : i;
...
void Purge::run(std::vector<std::ofstream>& extStreams, std::ofstream& extMovieStream)
{
  ...
  for (size_t step = 0; (step < Nsteps || autoSteps); ++step)
  {
    ...
    if (step % writeEvery == 0)
    {
      ...
      reverseGridData(Dpdt, Ncomp, Ngrid);
      reverseGridData(Dqdt, Ncomp, Ngrid);
      std::reverse(V.begin(), V.end());
      std::reverse(Pt.begin(), Pt.end());
      std::reverse(DTdt.begin(), DTdt.end());
      std::reverse(DTdtWall.begin(), DTdtWall.end());
      for (size_t j = 0; j < Ncomp; ++j) {
          std::reverse(components[j].Kl_values.begin(), components[j].Kl_values.end());
      }
...
std::string movieScriptTemplatePr(std::string s)
{
...
void Purge::createMovieScriptColumnV()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("V");
...
void Purge::createMovieScriptColumnKl()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("Kl");
...
void Purge::createMovieScriptColumnT()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("T");
...
void Purge::createMovieScriptColumnTw()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("Tw");
...
void Purge::createMovieScriptColumnPt()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("Pt");
...
void Purge::createMovieScriptColumnQ()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("Q");
...
void Purge::createMovieScriptColumnQeq()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("Qeq");
...
void Purge::createMovieScriptColumnP()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("P");
...
void Purge::createMovieScriptColumnPnormalized()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("Pnorm");
...
void Purge::createMovieScriptColumnDpdt()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("Dpdt");
...
void Purge::createMovieScriptColumnDqdt()
{
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
  ...
  makeMovieStream << "cd -- \"$(dirname \"$0\")\"\n";
#endif
  makeMovieStream << movieScriptTemplatePr("Dqdt");
